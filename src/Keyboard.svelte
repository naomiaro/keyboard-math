...
<svg xmlns="http://www.w3.org/2000/svg" xml:space="preserve" width="{octaveWidth}" height="{whiteKeyHeight}" bind:this={svg}>
<!--      
...
{#each Array(7) as _, row}
	<rect style="fill:white;stroke:black" x="{W*row}" y="0" width="{W}" height="{whiteKeyHeight}"/>
{/each}

<!--  Black keys (overlap with the white keys)  -->
<!-- 	The narrow part of white keys C, D and E is W - B*2/3 -->
{#each Array(2) as _, row}
	<rect style="fill:black;stroke:black" x="{nar3*(row+1)+B*row}" y="0" width="{B}" height="{blackKeyHeight}"/>
{/each}
	
<!-- 	The narrow part of white keys F, G, A, and B is W - B*3/4 -->
{#each Array(3) as _, row}
	<rect style="fill:black;stroke:black" x="{W*3+nar4*(row+1)+B*row}" y="0" width="{B}" height="{blackKeyHeight}"/>
{/each}
</svg>
